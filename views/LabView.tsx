...
export const LabView: React.FC = () => {
  ...
  const getCommentCount = (postId: string) => comments.filter(c => c.postId === postId).length;
        ...
        <div className="flex flex-wrap items-center gap-6 py-3 px-4 border-y border-slate-800 bg-slate-900/20 rounded-lg backdrop-blur-sm text-xs font-mono text-slate-400">
          <div className="flex items-center gap-2 text-slate-300">
            <div className="relative flex h-2 w-2">
              <span className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${activeExperiments > 0 ? 'bg-emerald-400' : 'bg-slate-600'}`}></span>
              <span className={`relative inline-flex rounded-full h-2 w-2 ${activeExperiments > 0 ? 'bg-emerald-500' : 'bg-slate-500'}`}></span>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-8 space-y-6">
          {notes.length > 0 ? (
                 ...
                 <div className="p-6">
                   ...
                   <div className="flex items-center justify-between pt-4 border-t border-slate-800/50">
                     ...
                     <div className="flex gap-4 text-slate-500 text-xs font-medium">
                        <span className="flex items-center gap-1 hover:text-pink-500 transition-colors cursor-pointer" title="Likes">
                          <TrendingUp size={14} /> {(note.likes || []).length}
                        </span>
                        <span className="flex items-center gap-1 hover:text-indigo-400 transition-colors cursor-pointer" title="Comments">
                          <MessageCircle size={14} /> {getCommentCount(note.id)}
                        </span>
                     </div>
                   </div>
                 </div>
            ...
            <div className="p-12 rounded-2xl border border-dashed border-slate-800 bg-slate-900/20 text-center flex flex-col items-center justify-center h-64">
               <FlaskConical className="text-slate-700 mb-4" size={48} />
               <h3 className="text-lg font-bold text-white mb-2">The Lab is Quiet</h3>
               <p className="text-slate-500 mb-6 max-w-md">
                 No experiments have been published yet. Be the first to share your findings with the network.
               </p>
               <Button onClick={handleCreateClick}>Start an Experiment</Button>
            </div>
          )}
        </div>
            ...
            <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-6">
              ...
              <ul className="space-y-4">
                {comments.slice(0, 3).map(c => (
                    <li key={c.id} className="group cursor-pointer relative pl-4 border-l-2 border-slate-800 hover:border-indigo-500 transition-colors">
                        <div className="text-sm font-medium text-slate-300 group-hover:text-primary transition-colors">
                           {c.author.name} commented
                        </div>
                        <div className="text-xs text-slate-500 mt-1 truncate">{c.content}</div>
                    </li>
                ))}
                {comments.length === 0 && <li className="text-xs text-slate-500">No recent activity.</li>}
              </ul>
            </div>
          )}
